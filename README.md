...
界面采用中文浅色工作台，以关系图为主要入口：

- 项目可手填路径或从系统目录窗口选择。浏览器会记住最近五个项目，下次打开时自动恢复上次项目；切换项目仍会保护未保存修改。没有可用系统窗口时可继续粘贴路径。
- 关系图、时间线与任务列表可分别开关，通过面板上的前移、后移按钮排序，切换半宽／全宽，并用高度滑块调整阅读区域。至少保留一个视图；布局保存在当前浏览器中。
- 布局预设提供“专注关系”“关系与时间对照”和“纵向总览”。任务列表内提供搜索、状态、标签、排序和更多筛选条件，筛选结果同步作用于三个视图；详情在右侧显示，窄屏使用抽屉。
- 关系图默认按人工关系的连通分量自动分组，筛选不重新排列剩余节点。箭头表示方向，虚线表示无向关联，同一对任务的多条关系分开绘制。自动布局不写入项目文件。
- 切换“手动布局”后可以拖动节点并保存位置，已有手动位置仍保留。支持触摸拖动、“显示全部”和“定位选中”；从选中任务可以直接填写关系起点。
- 时间线按实际时间计算桶内起止位置，默认显示最近 7 个时间桶。可切换 7、14、30 个时间桶，通过前后翻页或位置滑块连续浏览历史；选中任务时定位其观测区间。任务名称固定在左侧。
- 时间区间表示**创建至最近更新的观测跨度，不代表连续工作时长**。无效时间仍保留在列表中，但不绘制、不参与计数。

浏览器验收使用脱敏数据。启动 `tests/fixtures/http_fixture_server.py` 和前端开发服务后，以 `/tmp/codexflow-fixture-project` 作为测试项目，可运行 `tests/e2e/workspace_check.py` 检查多节点关系、时间轴对齐、布局持久化、窄屏溢出和触屏拖动保存；截图默认写入 `/tmp/codexflow-workspace/`。

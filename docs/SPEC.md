...
- 同一 Project 内按编辑目标保留草稿，切换 Conversation 或人工关系不会丢弃其他目标的草稿；
- 草稿只保留在当前页面内存中，不提供跨页面重载的恢复；存在草稿时，刷新或关闭网页应给出浏览器原生离开提示；
- 保存期间允许继续编辑，保存成功只确认该次提交包含的内容，后续修改仍处于 dirty 状态；
- 同一 Project 的写入按顺序执行，下一项使用上一项返回的文件版本；多项保存不是整体原子提交，任一项失败就停止，保留已保存结果及其余草稿；
- 节点拖拽保存失败的位置也属于草稿，参与刷新前的保存、丢弃和取消处理；
- 新建人工关系保存期间产生的后续修改，应继续附着在保存结果的稳定 Edge ID 上；
- 切换 Project 前有草稿时也提供保存、丢弃和取消，保存失败不发起切换；旧 Project 的异步结果不能更新新 Project。
...
冲突操作以点击时当前 Project 的全部待保存草稿为范围，并显示草稿数量；后续输入不并入这一次操作。保存副本成功不清除 dirty 状态，副本仍须通过完整图谱校验。明确覆盖仍逐项执行，后续项使用最新返回的版本，新出现的外部冲突必须再次处理。重新加载应明确标为丢弃全部草稿，且仅在加载成功后清除草稿；加载失败时保留。

刷新行为：

- dirty 时必须显示保存、丢弃、取消；
- 保存失败或产生冲突时停留在 dirty 状态；
- 保存并刷新只提交点击时捕获的草稿内容，如果保存期间产生了新草稿，暂停刷新并让用户再次选择；
- 丢弃只丢弃尚未提交的前端修改；
- 取消不刷新来源，也不改变当前页面状态；
- clean 时发现外部文件变化可以直接重新加载，但仍须重新验证 YAML。

刷新来源后保留当前筛选、选中身份和画布缩放、平移。切换时间桶或时区时，如果读取发现外部图谱版本变化且仍有草稿，保留原快照并进入显式冲突处理，不能悄悄更新草稿的保存基线。

...
共 24 张工单、31 条直接阻塞关系。其中 23 张标记为 ready-for-agent，1 张人工样本复核标记为 ready-for-human。标签表示任务类型，能否开始还取决于阻塞项是否完成。

V1-23 配置 Jev 关系分析；新增的 V1-24 为会话总结和工作流命名配置共用的 OpenAI 兼容文本生成服务，补充并覆盖 V1-09 与 V1-15 中仅使用 Codex 临时会话的限制。Jev 仍负责关系及证据选择。V1-24 使用用户填写的 API Key、Base URL 和模型名称，密钥存入 macOS 钥匙串。
...
文本生成入口：[V1-24：使用用户指定的文本模型生成会话总结与工作流名称](https://github.com/jinghonh/CodexFlow/issues/35)，无前置项，可立即开始。
...
24. **[V1-24：使用用户指定的文本模型生成会话总结与工作流名称](https://github.com/jinghonh/CodexFlow/issues/35)**

   阻塞项：无，可立即开始。

   交付：用户填写一组共用的 API Key、Base URL 和模型名称后，可以通过 OpenAI 兼容服务生成会话总结与工作流名称；关系判断仍由 Jev 负责，分组成员仍由既有规则决定。
...
flowchart TD
  ...
  T24["24 · 使用用户指定的文本模型生成会话总结与工作流名称"]
  T1 --> T2
  T2 --> T3
  T3 --> T4
  T4 --> T5
...
| [24](https://github.com/jinghonh/CodexFlow/issues/35) | 新增后续需求：用户配置文本模型生成会话总结与工作流名称 | 服务设置、总结和命名验收 |

- 44 条用户故事与 22 个核心验收场景全部覆盖。
- V1-24 是对已交付总结和命名能力的后续需求；其独立验收范围记录在 #35，不改写父规格 #11。
- 依赖图无环，且无重复的间接依赖。

...
type ProjectGraph = { project: { id: string; name: string }; nodes: GraphNode[]; relations: Relation[]; diagnostics: Diagnostic[] };
...
export function ProjectGraphView({ projectId, refreshVersion }: { projectId: string; refreshVersion: number }) {
      ...
      .then(async (next) => {
        ...
        if (active) {
          ...
          setSelection((current) => current &&
            (current.type === "edge" ? next.relations.some((relation) => relation.id === current.id) : next.nodes.some((node) => node.id === current.id))
            ? current : null);
        }
      })
  ...
  const detail = useMemo(() => {
    ...
    if (selection.type === "edge") {
      ...
      return relation ? { type: "edge" as const, relation } : null;
    }
  ...
  return <section id="relations" className="panel graph-panel">
    ...
    <h2>{graph?.project.name ?? "结构关系"}</h2>
    <p className="panel-intro">箭头从来源父会话指向后续会话。拖动画布平移，使用滚轮或控件缩放，点击节点或关系查看详情。</p>
    {error && <div className="page-error" role="alert">{error}</div>}
    {!graph && !error && <p className="empty-list">正在读取项目关系图…</p>}
    {graph && <>
      <div className="graph-summary"><strong>{graph.nodes.filter((node) => !node.referenceOnly).length} 条会话</strong><span>{graph.relations.length} 条观察关系</span><span>{graph.nodes.filter((node) => node.referenceOnly).length} 个仅有引用的端点</span></div>

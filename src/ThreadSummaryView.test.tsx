...
test("部分读取时保留旧总结和版本并禁止生成确定性新总结", async () => {
  vi.mocked(invoke).mockImplementation(async (command) => {
    if (command === "get_summary_preview") return {
      model: "test-model", characterLimit: 40000, characterCount: 100,
      totalFacts: 0, includedFacts: 0, totalMessages: 1, includedMessages: 1,
      truncated: false, turnsComplete: false, itemsComplete: false, sourceCurrent: false,
      readError: "第二页读取失败",
      contentAvailable: false, cacheCurrent: false,
      staleReason: "来源历史仅部分可读，旧总结保留，完整读取后才可重分析。",
      analysisBlockedReason: "来源历史不完整；保留旧总结，完整读取后再分析。",
      cachedSummary: { content: { goal: "旧目标", activity: "活动", outcome: "结果", decisions: "决定", issues: "问题" },
        evidenceIds: [], model: "test-model", createdAtUnixMs: 1000, sourceUpdatedAt: 200, inputDigest: "abcdef012345" },
    };
    if (command === "get_latest_summary_run") return null;
    throw new Error(`Unexpected command ${command}`);
  });
  render(<ThreadSummaryView threadId="thread-h" connected revision={0} onLocate={vi.fn(async () => {})} />);
  expect(await screen.findByText("旧目标")).toBeTruthy();
  expect(screen.getByText(/总结过期：来源历史仅部分可读/)).toBeTruthy();
  expect(screen.getByText("读取失败：第二页读取失败")).toBeTruthy();
  expect(screen.getByText(/来源更新版本 200 · 输入版本 abcdef012345/)).toBeTruthy();

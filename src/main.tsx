...
export function App() {
  ...
  const [threadMatches, setThreadMatches] = useState<ThreadMatches | null>(null);
  ...
  useEffect(() => {
    ...
    if (!projectId || showUnassigned) { setWorkstreams(null); return; }
    let active = true;
    setWorkstreams(null);
    invoke<WorkstreamView>("get_project_workstreams", { projectId })
      .then((value) => { if (active) setWorkstreams(value); })
      .catch((error) => { if (active) setQueryError(errorText(error)); });
    return () => { active = false; };
  }, [projectSessions?.project.id, showUnassigned, graphVersion]);

  useEffect(() => {
    const projectId = projectSessions?.project.id;
    if (!projectId || showUnassigned) { setThreadMatches(null); return; }
    let active = true;
    ...
    const timer = window.setTimeout(() => {
      ...
      } }).then((value) => { if (active) { setThreadMatches(value); setQueryError(""); } })
        .catch((error) => { if (active) setQueryError(errorText(error)); });
    }, 180);
  ...
  const visibleIds = React.useMemo(() => new Set(threadMatches?.matches.map((item) => item.threadId) ?? []), [threadMatches]);
  ...
  const selectionHidden = !!selectedThread && (showUnassigned || !!threadMatches) && !visibleThreads.some(({ thread }) => thread.id === selectedThread.id);
    ...
    <main className="content">
      ...
      <div className="page-body">
        ...
        <section id="sessions" className="panel session-panel">
          ...
          {queryError && <p className="page-error" role="alert">{queryError}</p>}
